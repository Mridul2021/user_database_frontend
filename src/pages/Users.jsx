...
const Users = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await axios.get(`https://user-database-manager.onrender.com/allUsers?page=${currentPage}`);
  ...
  const handleSearch = () => {
    axios
      .get(`https://user-database-manager.onrender.com/searchUsers?name=${searchName}&page=${currentPage}`)
      .then((result) => {
        setUsers(result.data.users);
        setTotalPages(result.data.totalPages);
  ...
  const handleDelete = (id) => {
    axios
      .delete(`https://user-database-manager.onrender.com/deleteUser/${id}`)
      .then(() => {
        setSelectedUsers((prevSelected) => prevSelected.filter((userId) => userId !== id));
        return axios.get(`https://user-database-manager.onrender.com/allUsers?page=${currentPage}`);
      })
      .then((result) => {
        setUsers(result.data.users);
        setTotalPages(result.data.totalPages);
  ...
  const handleDeleteSelected = () => {
    Promise.all(selectedUsers.map((id) => axios.delete(`https://user-database-manager.onrender.com/deleteUser/${id}`)))
      .then(() => {
        return axios.get(`https://user-database-manager.onrender.com/allUsers?page=${currentPage}`);
      })
      .then((result) => {
